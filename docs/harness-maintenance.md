...
- **branch protection은 없다**(private+Free repo) — `guard.sh` 훅이 main/develop 직접커밋·맨손 gh 머지를
  차단으로 대체. 합산 안전망 = `.githooks/pre-commit`(dogfooding) + ci-gate + PR 관행. public/Pro로 가면 protection 적용 검토
