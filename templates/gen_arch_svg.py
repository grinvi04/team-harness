...
def line(x1, y1, x2, y2, text="", dash=False, lx=None, ly=None):
    ...
    return (
        ...
        f'stroke="{stroke}" stroke-width="{sw}" {d} marker-end="{marker}"/>' + label_svg
    )
...
def curve(x1, y1, cx1, cy1, cx2, cy2, x2, y2, text="", lx=None, ly=None, dash=False):
    ...
    return (
        ...
        f'marker-end="{marker}"/>' + label_svg
    )


# ─── Edge helpers: box boundary shortcuts ────────────────────────────────────
def r(cx, cy):
    return cx + BW // 2, cy  # right edge center


def l(cx, cy):  # noqa: E743
    return cx - BW // 2, cy  # left edge center


def t(cx, cy):
    return cx, cy - BH // 2  # top edge center


def b(cx, cy):
    return cx, cy + BH // 2  # bottom edge center
...
def gen_example(out_path="docs/architecture.svg"):
    ...
    svg = wrap(
        W, H, "My Project — Architecture", "Example 3-node pipeline", nodes_svg + edges_svg, leg
    )
