...
- **branch protection 적용됨**(전 repo public 전환 #73 후 — decisions "브랜치 보호 표준"·"enforce_admins false→true 정정"):
  main·develop에 required status checks + force-push/삭제 차단 + 대화 resolve · 승인요건 0 · enforce_admins=true(서버 우회불가 계층0).
  합산 안전망 = **branch protection(서버 강제)** + `guard.sh`·`.githooks/pre-commit`(로컬 dogfooding) + ci-gate + PR 관행.
  적용/검증: `plugins/harness-guard/scripts/set-branch-protection.sh <repo> [--check]`.
