...
| [G] 문서 단일출처 정합 | ✅ **해소(2026-07-08)** | 포인터화 ✅ 배선(guard.sh:9-27 등 decisions.md 단일출처 포인터). intro 버전스탬프 드리프트(v0.24.2→0.29.28) ✅ 교정. **80KB 분할=하지 않음(재프레이밍)**: 로드맵 "8-10KB"는 오류(실측 96.5KB/190줄, ~10배 틀림) — 그러나 decision 로그를 크기로 쪼개면 단일출처가 파편화(=[G]의 취지에 역행)하므로 크기 무관 **존치**가 맞다(네비게이션은 헤더·이 표로 충분). grep가드=**안 만듦(speculative)**: 정책 중복은 semantic 속성이라 lexical grep은 brittle·오탐, 정합 회귀는 `/harness-review` 워크플로가 이미 커버 |
| [H] 워크플로 핸드오프 | ✅ **해소(2026-07-08)** | plan mode·milestone 흡수됨(기존). **hasSpec 정밀화 ✅ 확정**: 브랜치명↔스펙 파일명 1:1 정확매칭(guard.sh:198-200, `feature/<name>`→`docs/specs/<name>.md`). **plan↔F5 write 시점 ✅ 비갭**: F5는 `-f`로 working-tree 파일존재만 보므로(guard.sh:200, git-commit 무관) plan이 스펙을 디스크에 쓰면 F5 통과→feature-add가 코드와 함께 커밋, 데드락 없음. **milestone gitignore ✅ 교정**: team-harness 자기 `.gitignore`에 `docs/goals/`·`docs/milestones/` 추가(자기 표준 dogfood). **spec 수명=존치(재프레이밍)**: 완료 스펙은 decisions.md가 `spec: X.md`로 참조하는 영구 기록이라 삭제 시 포인터 깨짐 → 존치가 by-design |
...
- **Phase 4 (문서위생): ✅ 종결(2026-07-08).** `[G]`(포인터화 배선·intro 스탬프 교정·80KB분할은 단일출처 파편화라 존치·grep가드는 speculative로 제외) · `[H]`(hasSpec 1:1 정밀 확정·plan↔F5 비갭·milestone gitignore dogfood 교정·spec 존치). **잔여 없음 — 로드맵 Phase 1~4 전부 종결.**
