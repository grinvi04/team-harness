...
- **기존 repo 드리프트 점검**: `/repo-sync` 스킬(설치 후 어디서나) 또는
  ...
  **필수 자산이 빠지면 exit 1**(드리프트), WEAK(sentinel 없음)/WARN(룰 없음)은 경고(exit 0).
  ...
  단, 커밋 메시지 강제 체인(commitlint config·commit-msg hook·validator)은 정책 코어이므로 sentinel
  존재만 보지 않고 team-harness 정본의 정규화된 SHA-256과 일치해야 `OK`다. 대상 파일을 실행하지 않아
  드리프트 검사 중 소비 repo 코드가 실행되지 않는다.
