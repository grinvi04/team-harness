...
- **플러그인(가드·커맨드·스킬·에이전트)**: 로컬-디렉터리 마켓플레이스라 각 소비 repo는 **캐시된 버전을 실행**한다
  (`~/.claude/plugins/cache/team-harness/harness-guard/<버전>`). 버전 업 후 **갱신해야 실린다**:
  `/plugin marketplace update team-harness`(디렉터리 소스 재읽기) → `/plugin` 메뉴에서 harness-guard 업데이트.
...
- **기존 repo 드리프트 점검**: `/repo-sync` 스킬(설치 후 어디서나) 또는
  ...
  **필수 자산이 빠지면 exit 1**(드리프트), WEAK(sentinel 없음)/WARN(룰 없음)은 경고(exit 0).
  ...
  단, 커밋 메시지 강제 체인(commitlint workflow의 명시적 `.cjs` config 경로·commitlint config·commit-msg
  hook·validator)은 정책 코어이므로 action 이름이나 sentinel 존재만으로 `OK` 처리하지 않는다. config·hook·
  validator는 team-harness 정본의 정규화된 SHA-256과 일치해야 하며, 대상 파일을 실행하지 않아 드리프트
  검사 중 소비 repo 코드가 실행되지 않는다.
