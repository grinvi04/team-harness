...
문서 체계(11종)는 프로젝트 시작 전 단계로는 완결 상태다 — **추측성 선행 작성 금지.**
아래 시점이 오면 해당 문서를 그때 추가한다:

| 트리거 | 추가할 문서 |
|---|---|
| **스택 확정** | 스택 스캐폴드(AGENTS.md 빌드·테스트 명령 구체화, ci-gate 실제 단계, 스택 전용 가드 훅), 프론트엔드 컨벤션(디렉토리·상태관리·API 클라이언트 생성 규칙), 환경변수·설정 프로파일 규약 |
| **도메인 설계 시작** | 용어집(유비쿼터스 랭귀지 — 모듈 경계 판단의 기준), 공통코드·기준정보 거버넌스(코드 추가 권한·절차) |
