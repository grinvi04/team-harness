...
- **AC-15 (source·session credential trust):** WHEN live pilot이 source plugin과 모델 session을 실행하면,
  ...
  high-signal credential 파일 및 활성 `CODEX_HOME/auth.json` upload는 balanced nested shell parameter
  operator와 lexical dot segment를 포함한 동치 경로까지 실제 source option·stdin·positional source에 결합해 차단하고
