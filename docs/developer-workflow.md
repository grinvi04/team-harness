...
team-harness는 선택한 개발 방법론에 프로젝트 기준·검사·Git 완료 조건을 연결한다.
일반 설계·TDD·디버깅은 설치된 Superpowers 등 방법론 하나로 수행하고 중복 절차를 시작하지 않는다.

개인은 [개발 흐름](development-coordination.md)에 따라 기존 스택 기준·설정·검사 명령을 재사용하며 시작한다.
아래 브랜치·PR·서버 보호 절차는 GitHub에 연결한 프로젝트 기준이다. 로컬 전용 제품에는 원격 생성·공개를
강요하지 않고 제품의 Git 규칙과 검사·진행 문서를 사용한다.
...
| 기존 기능 확장 | `develop` | `plan` → `feature-modify` | 변경 동작은 GREEN, 기존 동작도 유지됨 |
| 버그 수정 | `develop` | `feature-modify` | 재현 테스트와 수정이 `fix/*`에 생성됨 |
| 원인 불명 실패 진단 | 현재 브랜치 | `systematic-debugging` | 재현·가설 판별로 근본 원인 또는 미확정 증거가 보고됨 |
| 완료 주장 재검증 | 어느 브랜치든 가능 | `verification-before-completion` | 현재 worktree·HEAD 기준 `VERIFIED / NOT VERIFIED` 판정 |
| 기능·수정 머지 | `feature/*` 또는 `fix/*` | `feature-merge` | 리뷰·CI 게이트 후 `develop` 반영 |
| 반복되는 CI·lint 수정 | 작업 브랜치 | `loop` | 지정 명령이 exit 0이 될 때까지 반복. 맥락 자동 선택이면 명시적 요청 없이 commit하지 않음 |
...
도구 화면에 slash command 입력창이 없어도 목표를 자연어로 요청하면 description의 사용·제외 경계에 따라
적합한 skill이 자동 선택될 수 있다. 중요한 작업은 “`feature-add` 절차로 주문 취소 기능을 개발해줘”처럼
skill 이름을 함께 말해도 된다. 자동 선택 여부와 현재 phase는 작업 업데이트에서 확인한다.
...
중요한 미결정이 해소되고 방향이 승인되면 개발 단계로 넘어간다. 기존 승인과 계획은 재사용한다. 오탈자처럼 결과가 자명한 작은 변경에는
별도 plan을 만들지 않아도 된다.
...
AI가 자동으로 commit·push·PR을 수행해도 커밋은 다음 형식을 사용한다.

```text
fix(auth): 만료 토큰 재사용 차단
...
- 기능 확장은 `feature/*` 브랜치를 사용하므로 먼저 `plan`으로 spec을 작성·승인해야 한다. 결과가 자명한
  trivial 확장만 명시적 면제로 plan을 생략할 수 있다.
- 명백한 버그라면 재현 테스트를 먼저 추가하고 `fix/*` 브랜치에서 수정한다.
- 수정 범위 밖에서 발견한 개선점은 현재 변경에 섞지 않고 별도 Issue나 작업으로 분리한다.

### 원인이 불분명하면 `systematic-debugging`

오류 메시지만 보고 바로 패치하지 않는다. `systematic-debugging`은 기대값·실제값과 재현 명령을 고정하고,
사실과 추론을 분리한 뒤 최대 3개 가설을 하나씩 판별한다. 근본 원인이 확인되기 전에는 코드를 바꾸지 않으며,
수정 요청일 때만 자동 회귀 테스트 또는 가장 작은 실행 가능 검사의 RED를 만든 뒤 `feature-modify` 계약으로
넘어간다. “원인만 찾아줘”라고 요청하면 작업트리는 바뀌지 않는다.

### 완료라고 말하기 전에 `verification-before-completion`

이 스킬은 “테스트가 통과했을 것”과 “현재 변경에서 실제 통과했다”를 구분한다. 완료·PR 준비·머지 준비·
릴리즈 준비 주장을 대상 테스트, 전체 품질, diff, git 상태, CI·배포 대상 SHA의 새 증거에 연결한다.
필수 검증이 실패하거나 실행되지 않았으면 `NOT VERIFIED`로 판정한다. 검증 실패를 직접 고치거나 머지하는
권한은 이 스킬에 없으며, 호출한 workflow로 결과를 돌려준다.
...
일반 작업은 Claude Code와 Codex가 skill description을 보고 의미에 맞는 절차를 선택한다. 반면 의도 라우터는
키워드 분류기가 아니라 현재 Git/PR 상태에 따라 이미 시작된 작업의 다음 단계를 이어 주는 장치다.

의도 라우터는 현재 브랜치와 열린 PR 상태를 읽어 다음 절차를 안내한다. 예를 들어:

- `docs/specs/`에 spec이 있고 구현 전이면 `feature-add`
...
라우터와 자동 선택은 편의를 위한 안내 장치다. 최종 판단 기준은 현재 상태와 각 skill의 수용기준이며, 적용 중인
skill과 phase는 작업 업데이트에서 확인할 수 있어야 한다. 라우터는 spec 파일의 존재만 확인하며 plan의
사람 승인 완료 여부까지 판정하지 않는다. 따라서 `feature-add`로 넘어가기 전 승인 게이트 완료를 별도로
확인해야 한다. `solo-merge`도 리뷰를 건너뛰는 절차가 아니다.
...
- Claude의 skill `effort`와 역할별 Haiku/Sonnet/Opus 매핑은 Claude 런타임 계약이다. Codex는 사용자에게
  승인된 native 역할의 모델·effort를 따른다. 명확한 저위험 구현에는 적합한 worker를 사용할 수 있다.
  예를 들어 승인된 Luna/max 구현 역할과 Luna/high 탐색 역할은 다른 배정이다. 현재 지원·상속과 실제
  실행값을 확인하며 설정을 임의 변경하지 않는다. 정본은 [모델 티어링](model-tiering.md)이다.
...
- [ ] 변경 내용을 내가 설명할 수 있다.
- [ ] 관련 로드맵·스펙 체크리스트·진행·사용 문서를 현재 변경·검사·미해결 항목과 대조하고 갱신했다.
- [ ] 완료·미완료·차단·다음 행동을 맞췄고, PR 또는 로컬 인계에 갱신 경로·근거(영향 없으면 이유)를 연결했다.
...
로컬 전용 개발은 PR·원격 CI·릴리즈 항목의 비적용 사유를 남긴다. 적용하지 않은 단계를 통과나 완료로 표시하지 않는다.
