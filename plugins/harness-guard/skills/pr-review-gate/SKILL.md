...
Codex에서는 `codex review --base <PR base>`로 변경분을 검토하고, 필요하면 `harness-verifier`와
`harness-security-reviewer` read-only subagent로 독립 반증을 추가한다. 인라인 코멘트 reply, review thread
resolve, 사람 승인, CI와 외부 commit-status 확인은 이 문서의 동일한 GitHub gate를 따른다.
...
Claude Code에서는 `/code-review`, Codex에서는 `codex review --base <PR base>`로 AI 리뷰를 수행한다.
사람 리뷰어의 인라인 코멘트도 같은 기준으로 처리한다.

---

## 1. AI 코드 리뷰 (`/code-review` 또는 `codex review`)

PR 생성 후 현재 도구의 AI review surface로 변경분(현재 브랜치/PR diff)을 리뷰하고,
...
```bash
bash ${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-$HOME/team-harness/plugins/harness-guard}}/scripts/pr-merge.sh "$PR"
```
