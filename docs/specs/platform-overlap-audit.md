...
- **In:** `plugins/harness-guard/skills/*/SKILL.md` 16개, Claude-facing agent 2개, Codex agent 정의 3개,
  `hooks.json` handler 4개, Codex 호환 실행 파일 10개의 전수 분류, 유지·축소·제거 우선순위, README·제품
...
- **AC-1 (전수성):** WHEN 저장소의 현재 구현 표면을 스캔하면 THEN 16개 skill, agent 정의 5개, hook handler
  4개, Codex 호환 실행 파일 10개가 감사 보고서에 각각 정확히 한 번 나타나야 한다. Codex 실행 표면은 파일명뿐
  아니라 Codex CLI나 호환 실행 파일의 직접 호출 관계로도 찾아야 한다.
