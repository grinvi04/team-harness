...
|---|---|---|---|---|
| `harness-explorer` | 코드베이스 탐색, QA 체크리스트, 근거 수집 | 부모 model 상속, `low` | `read-only` | 금지 |
| `harness-verifier` | 설계·회귀·테스트 누락 재검토 | 부모 model 상속, `high` | `read-only` | 금지 |
| `harness-security-reviewer` | 릴리즈/PR 보안 검토 | 부모 model 상속, `high` | `read-only` | 금지 |

custom agent의 `model`을 생략해 현재 사용자 플랜과 부모 session의 `/model` 선택을 상속한다. 역할 비용과
품질은 explorer=low, verifier/security=high reasoning으로 구분한다. 계정별 model slug나 전역
model/approval/sandbox 설정을 플러그인이 덮어쓰지 않는다. 구현·테스트·커밋은 주 Codex agent가 순차
실행한다. 병렬 subagent는 읽기 전용 독립 작업에만 사용한다.
