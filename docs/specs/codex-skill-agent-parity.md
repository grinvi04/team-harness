...
| `harness-verifier` | 설계·회귀·테스트 누락 재검토 | `gpt-5.6-terra`, `high` | `read-only` | 금지 |
| `harness-security-reviewer` | 릴리즈/PR 보안 검토 | `gpt-5.6-terra`, `high` | `read-only` | 금지 |

현재 Codex plan에서 확인된 `gpt-5.6-terra`를 사용하고, explorer는 medium, verifier/security는 high reasoning으로
역할을 나눈다. 계정별 전역 model/approval/sandbox 설정을 플러그인이 덮어쓰지 않는다. 구현·테스트·커밋은 주 Codex agent가
