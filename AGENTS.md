...
- 진행 상태가 여러 문서에 걸치면 기존 스펙·이슈·PR에 갱신 대상 경로를 연결한다. 종료 전에 그 범위의 미완료 표시를
  원본 증거와 대조하며, 병합·릴리즈 후 확인도 같은 담당자가 맡는다. 재사용 체크리스트를 실제 백로그로 세지 않는다.
...
- 이 repo PR은 `docs/ai-collaboration.md`의 `harness-doc-sync` 선언을 본문에 연결한다. 로컬 checker와 quality CI는 선언된 범위만 검사하며 의미·대상 누락은 독립 검토한다.
