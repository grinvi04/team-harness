...
- **In:**
  ...
  - `docs/onboarding.md`: 맨손 `git config core.hooksPath` 수동 단계를 **설정→검증**으로 승격(§B),
    **§C 로컬 테스트**에 배선 반증-스모크 참조 추가(team-harness 자기 검증). ※ §A.3(최종 검증)은
    소비 repo 신규셋업 맥락이라 team-harness-내부 스모크를 넣지 않는다 — 구현 중 정정.
...
- **영향 파일**: `tests/plugin-wiring-test.sh`(신규), `.github/workflows/ci-gate.yml`(등록),
  `docs/onboarding.md`(§B 수동단계→검증, §C 로컬 테스트에 스모크), `docs/troubleshooting.md`(:33 정정),
