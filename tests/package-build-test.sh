...
expected = {
    ...
    "harness-workflows": ("workflow-pack", 8),
}
errors = []
for folder, (unit, skill_count) in expected.items():
    ...
    if claude.get("version") != "0.69.0" or codex.get("version") != "0.69.0":
