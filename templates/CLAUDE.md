...
- git-flow 작업은 harness-guard 플러그인 커맨드 사용: `/plan`(feature 브랜치 전 `docs/specs/` 스펙 산출 — 가드가 스펙 없는 feature 브랜치 생성을 차단) → `/feature-add`·`/feature-modify` → `/feature-merge`, `/hotfix`, `/release`
