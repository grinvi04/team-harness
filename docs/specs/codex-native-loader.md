...
- **AC-8 (격리·복구):** WHEN loader/session pilot이 실행되면, the system SHALL 인증정보를 출력·커밋하지 않고
  임시 install/session 자산을 정리하며 repo와 사전에 기록한 사용자 Codex plugin/marketplace 상태를
  byte-equivalent하게 복구한다. 복구 검증 실패는 fail-closed로 보고한다.
...
- **AC-14 (binary trust continuity):** WHEN hardened launcher나 live pilot이 Codex를 실행하면, the system
  ...
  subprocess의 path·digest·device·inode·size identity를 실행 전후 검증한다. launcher의 sync·native 검사와
  최종 실행은 최초 검증 digest에 결박하며 PATH shadow·live override·검증 뒤 byte/inode 교체는 fail-closed한다.
...
- launcher와 pilot은 공용 binary trust helper를 사용하고, launcher가 호출하는 sync·native 검사도 검증된
  절대경로와 digest를 전달받아 각 Codex subprocess 전후 identity를 재확인한다.
- loader pilot은 Codex CLI를 fake하지 않는 통합 경로와 빠른 fixture 단위 테스트를 분리한다. 실제 모델 session은
  disposable Git repo에서 sentinel 불변과 hook-visible 결과만 검사한다.
...
| 3 | 격리 loader·새 session pilot과 report 계약 | AC-3,4,8,9 | `tests/codex-native-loader-pilot-test.sh`, `scripts/run-codex-native-loader-pilot.mjs`, `docs/pilots/` | fixture 테스트 + 승인된 실제 pilot | #1,2 | |
