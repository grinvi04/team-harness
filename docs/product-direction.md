...
8. [x] **Codex 공식 loader 전환:** monolith에 native manifest·command hooks·16개 skill wrapper를 직접 싣고
   ...
   operator-approved GitHub ref의 exact revision, 격리 HOME, refresh/API-key와 inherited long-lived credential
   환경이 없는 session credential에 결박한다. monolith 전환만 승인하며 split package의 `installable:false`와
   marketplace 승격 보류는 유지한다.
