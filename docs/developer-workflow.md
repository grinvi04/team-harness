...
> 브라우저에서 빠르게 훑으려면 [개발자 실행 가이드](developer-workflow.html), 처음 설치한다면
> [onboarding.md](onboarding.md), 전체 구조를 먼저 보고 싶다면 [intro.html](intro.html)을 읽는다.

**현재 기준:** harness-guard v0.56.0 · 스킬 16종. 이 Markdown이 내용 정본이며,
`developer-workflow.html`은 같은 절차를 작업 상태 중심으로 재구성한 읽기 화면이다.

## 문서를 고르는 가장 짧은 기준

| 지금 필요한 것 | 먼저 볼 문서 |
|---|---|
| team-harness가 무엇을 강제하는지 이해 | [전체 소개](intro.html) |
| 지금 브랜치에서 다음 행동 찾기 | [개발자 실행 가이드](developer-workflow.html) |
| 설치·초기 설정 | [온보딩](onboarding.md) |
| 정확한 절차와 완료 조건 확인 | 이 Markdown과 [`skills/`](../plugins/harness-guard/skills/) |
...
```text
systematic-debugging으로 이 CI 실패의 원인만 찾아줘. 아직 수정하지 마.
재현 명령, 확인된 사실, 기각한 가설, 근본 원인을 구분해 보고해줘.
```
...
```text
verification-before-completion으로 이 브랜치가 PR 준비됐다는 주장을 검증해줘.
현재 worktree와 HEAD SHA에 대해 새로 실행한 명령·종료 코드로 판정해줘.
```
...
마지막 체크는 기억에 의존하지 않는다. 완료를 선언하기 직전 `verification-before-completion`으로
현재 worktree·HEAD·CI SHA를 다시 대조한다.
...
| 현재 상태에서 다음 행동 찾기 | [developer-workflow.html](developer-workflow.html) |
