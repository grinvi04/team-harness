...
| `scripts/codex-hardened.sh` | 해당 없음 | CLI 시작 전 두 Codex cache patch를 fail-closed 실행 | Codex 운영 통제 (cmux CLI만) | `codex-hardened-launcher-test.sh` |
...
1. `~/.codex/config.toml`의 `approval_policy = "untrusted"`를 유지한다. 이는 모든 Codex의 비신뢰 명령에
   ...
   막는다고 주장하지 않는다(#283).
2. Codex에 `harness-guard` **v0.46.0 이상**을 설치/갱신한다. patcher는 cache에
...
`harness-guard` cache patch와 `security-guidance` adapter patch를 순서대로 적용하고, 둘 중 하나라도 실패하면
Codex를 실행하지 않는다. `approval_policy = "untrusted"`는 변경하지 않는다.
