...
| 6 | 관련 로드맵·체크리스트·진행·사용 문서 현행화 | 실제 변경·검사 결과·미해결 항목과 대조; 갱신 경로 또는 영향 없음 근거 |
...
관련 로드맵·스펙 체크리스트·진행 안내·직접 소비 문서를 확인해 갱신 대상을 정한다.
범위·단계 변경과 취소·완료를 해당 원본에 반영하고 과거 증거를 새 결과로 바꾸지 않는다.
